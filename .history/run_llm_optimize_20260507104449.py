...
def run_optimization(dataset_path, dataset_type="c1", max_rounds=5,
                     ...
                     eval_runs=1, seed_step=97, force=False):
    """主优化循环"""
    ...
    # 若已有 summary.json 或已有轮次结果，则跳过（除非 --force）
    ...
    if has_summary or has_rounds:
        if force:
            ...
            print(f"[Force] 已检测到旧结果，重新运行: {output_dir}")
        else:
            print(f"[Skip] 已有结果，跳过 (使用 --force 强制重新运行): {output_dir}")
            return None, None
