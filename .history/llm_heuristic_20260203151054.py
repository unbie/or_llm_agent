...
def generate_or_code_solver(messages_bak, model_name, data, max_attempts=3):
    ...
    while attempt < max_attempts:
        ...
        full_code = (
            ...
            f"{plugin_class_code}\n\n"
            # === 第三部分：LLM权重调整函数 ===
            "def llm_weight_adjuster_wrapper(solver_state):\n"
            "    '''封装LLM权重调整调用'''\n"
            "    import sys\n"
            "    import os\n"
            "    sys.path.insert(0, r'" + current_project_dir + "')\n"
            "    from llm_heuristic import query_llm_for_weight_adjustment\n"
            "    return query_llm_for_weight_adjustment(solver_state)\n\n"
            # === 第四部分：执行代码 ===
            ...
            "        # 注入LLM权重调整器\n"
            "        solver.llm_weight_adjuster = llm_weight_adjuster_wrapper\n"
            "        print('[Initialization] Solver initialized with LLM adjuster')\n"
