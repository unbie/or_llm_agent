...
def compute_code_change_ratio(old_code, new_code):
    """计算代码改动比例，用于限制大幅重写。返回 0~1 之间的比例。"""
    old_lines = [ln.strip() for ln in old_code.strip().splitlines() if ln.strip()]
    new_lines = [ln.strip() for ln in new_code.strip().splitlines() if ln.strip()]
...
def run_optimization(dataset_path, dataset_type="c1", max_rounds=5,
                     ...
                     eval_runs=1, seed_step=97, min_improve_pct=1.0,
                     max_change_ratio=0.35, confirm_runs=None,
                     confirm_iters=None, confirm_seed_step=None,
                     confirm_seed_offset=1000, force_run_large_change=False):
    """主优化循环"""
    ...
    confirm_runs = eval_runs if confirm_runs is None else confirm_runs
    confirm_iters = iters_per_round if confirm_iters is None else confirm_iters
    confirm_seed_step = seed_step if confirm_seed_step is None else confirm_seed_step

    print(f"评估重复: {eval_runs} 次 (seed_step={seed_step})")
    print(f"最小改进阈值: {min_improve_pct:.2f}%")
    print(f"代码改动上限: {max_change_ratio:.2f}")
    print(f"超限仍评估: {force_run_large_change}")
    print(f"确认评估: {confirm_runs} 次 (iter={confirm_iters}, seed_step={confirm_seed_step})")
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        prompt += (
            f"\n\n【改动约束】只做小幅修改，尽量保持结构不变。\n"
            f"不要新增/删除方法，也不要大幅重写。\n"
            f"本轮允许的改动比例上限约为 {max_change_ratio:.2f}。\n"
            f"若无法改进至少 {min_improve_pct:.2f}% ，请只做轻微参数调整。\n"
        )
        ...
        # 5. 代码改动幅度检查（避免大幅重写）
        change_ratio = compute_code_change_ratio(current_code, new_code)
        if change_ratio > max_change_ratio and not force_run_large_change:
            decision = "skipped"
            print(f"[Round {round_num}] 代码改动过大 (ratio={change_ratio:.2f} > {max_change_ratio:.2f})，跳过本轮")
            ...
            round_results.append({
                'round': round_num, 'type': decision, 'cost': best_cost,
                'reason': 'change_ratio_exceeded',
                'change_ratio': change_ratio,
            })
            continue
        elif change_ratio > max_change_ratio and force_run_large_change:
            print(f"[Round {round_num}] 代码改动过大 (ratio={change_ratio:.2f} > {max_change_ratio:.2f})，按设置继续评估")
        ...
        meets_threshold = new_cost <= prev_cost * (1 - min_improve_pct / 100.0)

        confirm_eval = None
        confirm_mean_cost = None
        confirm_required = confirm_runs > 0
        if meets_threshold and confirm_required:
            print(f"\n[Confirm] 通过阈值，进行确认评估 (runs={confirm_runs}, iter={confirm_iters})...")
            confirm_eval = evaluate_operator(
                data,
                new_code,
                max_iter=confirm_iters,
                seed=seed + confirm_seed_offset,
                eval_runs=confirm_runs,
                seed_step=confirm_seed_step,
                verbose=False,
            )
            if confirm_eval is None:
                print(f"[Round {round_num}] 确认评估失败，回退到上一版")
                confirm_mean_cost = None
            else:
                confirm_mean_cost = confirm_eval['mean_cost']

        confirm_ok = (not confirm_required) or (confirm_eval is not None and confirm_mean_cost is not None)
        final_cost = confirm_mean_cost if confirm_required else new_cost

        if meets_threshold and confirm_ok and final_cost <= prev_cost * (1 - min_improve_pct / 100.0):
            status = "✓ 改善"
            decision = "accepted"
            current_code = new_code
            best_cost = final_cost
            metrics = confirm_eval['best_metrics'] if confirm_eval is not None else new_metrics
        ...
        if confirm_eval is not None:
            print(f"  确认评估均值:     {confirm_eval['mean_cost']:.2f} ± {confirm_eval['std_cost']:.2f}")
        print(f"  决  定: {decision}")
        print(f"  历史最优(评估均值): {best_ever_cost:.2f}")

        # 记录历史（保存完整分析）
        history_cost = confirm_mean_cost if confirm_eval is not None else new_cost
        history.append({
            'round': round_num,
            'cost': history_cost,
            'prev_cost': prev_cost,
        ...
        with open(round_dir / "llm_improvements.json", 'w', encoding='utf-8') as f:
            json.dump({
                ...
                'confirm_mean_cost': confirm_mean_cost,
            }, f, indent=2, ensure_ascii=False, default=str)
        effective_eval = confirm_eval if (decision == "accepted" and confirm_eval is not None) else new_eval
        effective_metrics = effective_eval['best_metrics']
        with open(round_dir / "result.json", 'w', encoding='utf-8') as f:
            json.dump({
                'round': round_num,
                'type': decision,
            'cost': effective_eval['mean_cost'],
            'best_cost': effective_eval['best_cost'],
            'mean_cost': effective_eval['mean_cost'],
            'std_cost': effective_eval['std_cost'],
            'eval_runs': effective_eval['eval_runs'] if 'eval_runs' in effective_eval else eval_runs,
            'seeds': effective_eval['seeds'],
                'prev_cost': prev_cost,
                'change_pct': cost_change,
            'num_routes': effective_metrics['num_routes'],
            'elapsed_time': effective_metrics['elapsed_time'],
                'change_ratio': change_ratio,
                'min_improve_pct': min_improve_pct,
                'confirm_mean_cost': confirm_mean_cost,
                'confirm_eval_runs': confirm_runs,
                'confirm_iters': confirm_iters,
                'confirm_seed_step': confirm_seed_step,
            'metrics': {k: v for k, v in effective_metrics.items()
                           if k not in ('op_stats_raw',)},
            }, f, indent=2, ensure_ascii=False, default=str)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min-improve-pct', type=float, default=1.0,
                        help='接受改进的最小提升百分比（默认 1.0）')
    parser.add_argument('--max-change-ratio', type=float, default=0.35,
                        help='允许的最大代码改动比例（默认 0.35）')
    parser.add_argument('--confirm-runs', type=int, default=None,
                        help='确认评估的运行次数（默认等于 eval-runs）')
    parser.add_argument('--confirm-iters', type=int, default=None,
                        help='确认评估迭代次数（默认等于 iters）')
    parser.add_argument('--confirm-seed-step', type=int, default=None,
                        help='确认评估 seed 步长（默认等于 seed-step）')
    parser.add_argument('--confirm-seed-offset', type=int, default=1000,
                        help='确认评估 seed 基准偏移（默认 1000）')
    parser.add_argument('--force-run-large-change', action='store_true',
                        help='当代码改动超过阈值时仍然执行评估')
    ...
    if args.suite == 'core30':
        ...
        for dataset_type, dataset_name in suite:
            ...
            run_optimization(
                ...
                confirm_seed_step=args.confirm_seed_step,
                confirm_seed_offset=args.confirm_seed_offset,
                force_run_large_change=args.force_run_large_change,
            )
    else:
