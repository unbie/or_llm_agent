...
【数据结构】：
   ...
   - self.solver.calculator: 成本计算器（可计算完整路径成本，包括时间窗、货损等）

【成本计算说明】：
   框架提供了两种成本计算方式：
   
   1. **简化距离成本**（推荐在算子中使用）：
      - 只考虑距离增量：cost = dist(prev, node) + dist(node, next) - dist(prev, next)
      - 优点：计算快速，作为启发式指标效果良好
      - 缺点：不考虑时间窗、货损等复杂约束
   
   2. **完整成本计算**（通过 self.solver.calculator 使用）：
      ```python
      # 计算路径的完整成本（包括C12距离+C13制冷+C2货损+C3时间惩罚）
      route_nodes = [self.solver.id_to_customer[node_id] for node_id in route]
      cost_info = self.solver.calculator.calculate_route_cost(route_nodes, self.dist_matrix)
      total_cost = cost_info['variable_cost']  # C12+C13+C2+C3
      ```
      - 优点：精确考虑所有成本因素
      - 缺点：计算较慢，在大规模问题中可能影响性能
   
   **建议**：在算子的插入决策中使用简化距离成本（方法1），最终解的评估会通过
           框架的 cost() 函数自动使用完整成本计算（方法2）
...
**成本计算方法**：
方法1（简化）：只考虑距离增量
   - 插入成本 = dist(prev, node) + dist(node, next) - dist(prev, next)
   - 新路径成本 = dist(0, node) * 2
   
方法2（完整）：使用 self.solver.calculator.calculate_route_cost() 计算完整成本
   - 包括距离成本(C12)、制冷成本(C13)、货损成本(C2)、惩罚成本(C3)
   - 更精确但计算量更大
   
**本实现**：推荐使用方法1（距离增量），因为：
   1. 在插入阶段，时间窗和货损成本难以准确估算
   2. 距离是成本的主要组成部分，可作为良好的启发式指标
   3. 最终的解会通过 cost() 函数计算准确的完整成本
   
如需使用完整成本计算，可在算子中通过 self.solver.calculator 访问成本计算器
...
**成本计算**：同样使用距离增量作为启发式指标（原因见greedy_insert说明）
