...
def run_optimization(dataset_path, dataset_type="c1", max_rounds=5,
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if meets_threshold and confirm_runs > 0:
        ...
        if meets_threshold and (confirm_eval is None or confirm_mean_cost is not None):
            final_cost = confirm_mean_cost if confirm_mean_cost is not None else new_cost
            if final_cost <= prev_cost * (1 - min_improve_pct / 100.0):
                status = "✓ 改善"
                decision = "accepted"
