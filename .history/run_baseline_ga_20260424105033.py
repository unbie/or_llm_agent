...
# 与 run_batch_no_llm.py 保持一致的核心 12 实验配置
CORE12_SUITE = [
    ('c1',  'c101.txt', 3),
    ('c1',  'c105.txt', 3),
    ('c2',  'c201.txt', 3),
...
def build_experiment_suite(suite_name: str = 'core12', runs_per_instance: int = 3):
    """根据套件名构造批量实验列表。"""
    suite_name = (suite_name or 'core12').lower()

    if suite_name == 'core12':
        return [(t, inst, runs_per_instance) for t, inst, _ in CORE12_SUITE]
...
示例:
  ...
  python run_baseline_ga.py --all --pop 100 --gen 1000
    ...
    parser.add_argument('--suite',    default='core12', choices=['core12', 'full56'],
                        help='批量实例集: core12(默认) 或 full56(全56算例)')
    parser.add_argument('--runs',     type=int, default=3, help='每个实例运行次数 (默认3)')
