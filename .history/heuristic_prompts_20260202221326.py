...
class HeuristicPlugin:
    ...
    def worst_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 实现worst_removal算子
        # 关键改进点：
        # 1. 计算边际贡献时，不仅考虑距离，还要加入需求因子
        # 2. 综合评分公式：contrib = dist_contrib + demand * 0.01
        # 这样可以优先移除距离远且需求大的节点，提升改进空间
    ...
    def related_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 实现related_removal算子
        # 关键步骤：
        # 1. 随机选择一个种子节点
        # 2. 计算所有节点与种子节点的距离
        # 3. 按距离升序排序，选择最近的n个节点
        # 4. 移除这些相近的节点，便于后续重新优化它们的位置
    ...
    def shaw_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 实现shaw_removal算子
        # 关键改进点：
        # 1. 综合考虑距离、时间窗、需求三个维度的相似性
        # 2. 必须进行安全归一化，避免除零错误：
        #    if max_value < 0.001: max_value = 1.0
        # 3. Shaw分数公式：9*dist + 3*time + 2*demand（权重可调）
        # 4. 移除相似度高的节点，便于统一优化
    ...
    def history_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 实现history_removal算子
        # 关键步骤：
        # 1. 使用self.node_history字典记录节点的历史修改信息
        # 2. 如果历史为空，回退到random_removal
        # 3. 按历史值升序排序，优先移除长期未修改的节点
        # 4. 这样可以探索搜索空间中被忽略的区域
    ...
    def cluster_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 实现cluster_removal算子
        # 关键步骤：
        # 1. 创建5x5网格，将客户划分到不同网格单元
        # 2. 随机选择一个非空网格
        # 3. 如果该网格节点数不足，扩展到相邻网格
        # 4. 移除同一聚类的节点，便于重新规划该区域路径
    ...
    def greedy_insert(self, solution, removed_nodes):
        ...
        Returns:
            插入所有节点后的完整解
        '''
        # TODO: 实现greedy_insert算子
        # 【关键】必须添加容量约束检查！
        # 步骤：
        # 1. 对每个待插入节点，遍历所有现有路径
        # 2. 检查容量约束：current_load + node_demand <= capacity
        # 3. 只在满足容量约束的路径中寻找成本增量最小的位置
        # 4. 如果所有路径都超容量，则新建路径
        # 5. 这样可以避免生成不可行解
    ...
    def regret_insert(self, solution, removed_nodes):
        ...
        Returns:
            插入所有节点后的完整解
        '''
        # TODO: 实现regret_insert算子
        # 【关键】必须添加容量约束检查！
        # 步骤：
        # 1. 对每个待插入节点，找到最佳和次佳插入位置
        # 2. 计算后悔值：regret = second_best_cost - best_cost
        # 3. 优先插入后悔值大的节点（即如果不现在插入，后面代价会更高）
        # 4. 在寻找插入位置时，必须检查容量约束
        # 5. 这样既保证可行性，又提高插入质量
