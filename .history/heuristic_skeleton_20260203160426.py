...
class HeuristicSolver:
    ...
    def solve(self, max_iters=300, seed=None):
        ...
        T = max(current_cost * 0.15, 150) if current_cost < float('inf') else 200  # 初始温度（能接受差15%的解）
        
        # 自适应冷却系数：确保温度在整个迭代过程中缓慢下降
        # 目标：让算法在整个过程中都保持一定的探索能力
        # 使用更保守的冷却策略：最终温度是初始温度的1%
        target_ratio = 0.01  # 最终温度为初始温度的1%（非常缓慢的冷却）
