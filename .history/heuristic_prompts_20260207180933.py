...
【成本计算（极其重要，必须遵守）】：
⚠️ 禁止使用 dist_matrix 的距离增量 dist(a,c)+dist(c,b)-dist(a,b) 作为插入成本！
⚠️ 必须使用真实成本函数来计算插入增量！
...
self.solver 由框架在初始化后注入，在 greedy_insert/regret_insert 被调用时已可用。
calculate_route_cost() 返回一个字典，包含 variable_cost（综合成本，含运输、冷藏、新鲜度损失、时间窗惩罚）。
