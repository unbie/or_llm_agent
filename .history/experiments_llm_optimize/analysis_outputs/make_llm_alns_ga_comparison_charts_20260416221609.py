        ...
        rows.append(
            {
                ...
                "alns_std": stdev(alns_runs) if len(alns_runs) > 1 else 0.0,
                "ga_best": ga_best,
                "ga_mean": mean(ga_runs),
                "ga_std": stdev(ga_runs) if len(ga_runs) > 1 else 0.0,
...
def plot_best_cost_line(df: pd.DataFrame, out_png: Path) -> None:
    x = np.arange(len(df))

    fig, ax = plt.subplots(figsize=(10, 5.2))
    ax.plot(x, df["llm_best"], marker="o", linewidth=2.2, label="LLM优化算子(最优)", color="#0B3C6D")
    ax.plot(x, df["alns_best"], marker="s", linewidth=2.0, label="传统ALNS(最优)", color="#4C78A8")
    ax.plot(x, df["ga_best"], marker="^", linewidth=2.0, label="GA(最优)", color="#F58518")

    ax.set_xticks(x)
    ax.set_xticklabels(df["instance_label"])
    ax.set_ylabel("总成本")
    ax.set_title("三算法最优成本折线图", fontweight="bold")
    ax.grid(axis="y", alpha=0.25)
    ax.legend(frameon=False, ncols=3, loc="upper center", bbox_to_anchor=(0.5, 1.13))
...
def plot_stability_errorbar(df: pd.DataFrame, out_png: Path) -> None:
    x = np.arange(len(df))
    ...
    # 同图叠加 LLM 最优点，便于稳定性与最优解质量对照
    ax.scatter(x, df["llm_best"], marker="*", s=120, color="#0B3C6D", label="LLM优化最优")

    ax.set_xticks(x)
    ax.set_xticklabels(df["instance_label"])
    ax.set_ylabel("总成本")
    ax.set_title("稳定性对比：均值+误差线（ALNS/GA三次重复）", fontweight="bold")
    ax.grid(axis="y", alpha=0.25)
    ax.legend(frameon=False, ncols=2, loc="upper center", bbox_to_anchor=(0.5, 1.16))
...
def main() -> None:
    ...
    chart3 = out_dir / "llm_alns_ga_best_cost_line.png"
    chart4 = out_dir / "llm_alns_ga_stability_errorbar.png"
    plot_cost_comparison(df, chart1)
    plot_gain_comparison(df, chart2)
    plot_best_cost_line(df, chart3)
    plot_stability_errorbar(df, chart4)
