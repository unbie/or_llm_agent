...
def plot_bar_and_line(df: pd.DataFrame, out_fig: Path):
    ...
    ax.bar(x + 1.5 * w, df["aco_best"], width=w, label="ACO+LS(best)", color="#54A24B")
    ax.set_xticks(x)
    ...
    ax.plot(x, df["aco_best"], marker="D", linewidth=2.0, label="ACO+LS(best)", color="#54A24B")
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Cost")
    ...
    ax.bar(x + w, df["llm_vs_aco_best_pct"], width=w, label="LLM vs ACO+LS", color="#2ca02c")
...
def write_markdown_report(
    ...
    md = f"""---
title: 四算法综合对比实验报告
description: 基于C101/C105/C201/C205对LLM优化算子、ALNS、GA、ACO+LS进行柱状图、折线图与多表格对比
author: OR-LLM Agent
ms.date: 2026-04-17
ms.topic: concept
keywords:
...
* 算法: LLM优化算子、ALNS基线、GA基线、ACO+局部搜索
...
* 分实例最优胜场: LLM={llm_wins}, ALNS={alns_wins}, GA={ga_wins}, ACO+LS={aco_wins}
