...
【生成代码后必须自检】：
✅ worst_removal: 是否包含 `contrib = dist_contrib + demand * 0.01`
...
【worst_removal】- 成功率提升的关键算子：
   - **核心公式（必须严格遵守）**：
       ...
       contrib = dist_contrib + demand * 0.01  # ← 必须加需求因子！
   - **为什么加需求因子**：纯距离贡献会导致算子选择过于单一，加入需求可以：
     * 打破平局（当多个节点距离贡献相同时）
     * 优先移除需求大的节点（为后续优化创造更多空间）
     * 系数0.01确保需求不会主导距离因素
    ...
    def worst_removal(self, solution, ratio):
        ...
        for route_idx, route in enumerate(solution):
            ...
            for pos_idx in range(1, len(route)-1):
                ...
                # 【必须实现】添加需求因素，系数0.01不可省略！
                demand = self.customer_lookup.get(node, {}).get('demand', 0)
                contrib = dist_contrib + demand * 0.01  # ← 关键公式，不可简化！
...
【代码完成后务必检查以下关键点】：
1. worst_removal 的 contrib 公式是否包含 `+ demand * 0.01`
