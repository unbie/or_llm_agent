...
class HeuristicSolver:
    ...
    def _fallback_greedy_insert(self, solution, removed_nodes):
        ...
        if not removed_nodes:
            return solution
        if not hasattr(self, '_logged_greedy'):
            print(f"[_fallback_greedy_insert] 使用{'真实成本函数' if self.calculator else '距离增量'}")
    ...
    def _fallback_regret_insert(self, solution, removed_nodes):
        ...
        if not removed_nodes:
            return solution
        if not hasattr(self, '_logged_regret'):
            print(f"[_fallback_regret_insert] 使用{'真实成本函数' if self.calculator else '距离增量'}")
            self._logged_regret = True
        if not solution:
            return [[0, node, 0] for node in removed_nodes]
