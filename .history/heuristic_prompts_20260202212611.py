...
class HeuristicPlugin:
    ...
    def worst_removal(self, solution, ratio):
        ...
        for route_idx, route in enumerate(solution):
            ...
            for pos_idx in range(1, len(route)-1):
                ...
                contrib = (self.dist_matrix[prev_node][node] + 
    ...
    def regret_insert(self, solution, removed_nodes):
        ...
        '''
        # TODO: 实现后悔插入逻辑
        # 1. 边界处理：removed_nodes 为空返回 solution
        # 2. 循环直到所有节点插入完毕：
        #    a. 对每个待插入节点，计算所有可能位置的成本增量
        #    b. 找出最佳位置（best_cost）和次佳位置（second_best_cost）
        #    c. 计算后悔值 regret = second_best_cost - best_cost
        #    d. 选择后悔值最大的节点
        #    e. 将该节点插入到其最佳位置
        # 3. 注意：新建路径的成本也要纳入考虑
        # 4. return solution
    ...
    def random_insert(self, solution, removed_nodes):
        ...
        # 1. 边界处理：removed_nodes 为空返回 solution
        # 2. 对每个待插入节点：
        #    a. 收集所有可行的插入位置 [(route_idx, pos), ...]
        #    b. 加入新建路径选项 (None, None)
        #    c. 随机选择一个位置
        #    d. 执行插入
        # 3. return solution
        pass
