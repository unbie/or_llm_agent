...
def generate_or_code_solver(messages_bak, model_name, data, max_attempts=3):
    ...
    while attempt < max_attempts:
        ...
        full_code = (
            ...
            "        best_sol, best_cost = solver.solve_multi_run(max_iters=600, num_runs=3, base_seed=42)\n"
        ...
        messages.append({
            ...
            f"【重要】你必须实现以下5个方法（缺一不可，框架会调用所有5个）：\n"
            f"  破坏算子: random_removal(self, solution, ratio), route_removal(self, solution, ratio), string_removal(self, solution, ratio)\n"
            f"  修复算子: greedy_insert(self, solution, removed_nodes), regret_insert(self, solution, removed_nodes)\n"
            f"不要实现 worst_removal！不要删除任何算子！5个方法都必须存在！\n"
            f"如果超时，可以在 regret_insert 中只比较每条路径的最佳插入位置，而不是所有位置。"
        })
        attempt += 1
