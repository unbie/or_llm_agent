...
class HeuristicPlugin:
    ...
    def _candidate_positions(self, route, node):
        """仅评估少量候选插入位置，加速计算。"""
        ...
        k = getattr(self.solver, 'candidate_list_size', 10) if self.solver is not None else 10
...
def run_batch_experiments():
    ...
    # 仅先跑 r/rc 类（如需全量，设为 False）
    run_only_r_rc = True
    if run_only_r_rc:
        experiments = [
            (t, inst, runs)
            for (t, inst, runs) in experiments
            if t in ['r1', 'r2', 'rc1', 'rc2']
        ]
