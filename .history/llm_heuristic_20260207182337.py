...
# 内置备用算子（LLM不可用时使用，确保可运行）
DEFAULT_PLUGIN_CODE = r"""
class HeuristicPlugin:
    def __init__(self, *args, **kwargs):
        self.dist_matrix = kwargs.get('dist_matrix')
        self.vehicle_capacity = kwargs.get('vehicle_capacity', 200)
        self.nodes_dict = kwargs.get('nodes_dict')
        self.solver = None  # 由 HeuristicSolver 注入

    def random_removal(self, solution, ratio):
        if self.solver and hasattr(self.solver, '_fallback_random_removal'):
            return self.solver._fallback_random_removal(solution, ratio)
        return solution, []
    ...
    def greedy_insert(self, solution, removed_nodes):
        if self.solver and hasattr(self.solver, '_fallback_greedy_insert'):
            return self.solver._fallback_greedy_insert(solution, removed_nodes)
        return solution

    def regret_insert(self, solution, removed_nodes):
        if self.solver and hasattr(self.solver, '_fallback_regret_insert'):
            return self.solver._fallback_regret_insert(solution, removed_nodes)
        return solution
"""
...
def query_llm(messages, model_name="ep-20260106214023-k4p8b", temperature=0, timeout_s=60):
    """
    ...
    try:
        ...
        for chunk in response:
            if time.time() - start_time > timeout_s:
                raise TimeoutError("LLM streaming timeout")
            # 检查是否有 choices
...
def generate_or_code_solver(messages_bak, model_name, data, max_attempts=3):
    ...
    while attempt < max_attempts:
        llm_response = query_llm(messages, model_name, timeout_s=60)

        if not llm_response.strip():
            print("[Warning] LLM响应为空或超时，使用内置算子")
            plugin_class_code = DEFAULT_PLUGIN_CODE
        else:
            # === 提取 LLM 生成的代码 ===
        ...
        full_code = (
            ...
            "    try:\n"
            "        pl这ugin = HeuristicPlugin(data=data)\n"
            "        print('[Initialization] Plugin initialized')\n"
            "        solver = HeuristicSolver(data, plugin)\n"
            "        print('[Initialization] Solver initialized')\n"
            "        best_sol, best_cost = solver.solve_multi_run(max_iters=300, num_runs=3, base_seed=None)\n"
...
if __name__ == "__main__":
    ...
    solomon_file = r"D:\pythonProject\or_llm_agent\data\1 Solomon Benchmark\r2\r201.txt"

    dataset = load_solomon_data(solomon_file)
    dataset['force_cost_insert'] = True
