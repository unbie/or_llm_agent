...
class HeuristicPlugin:
    ...
    def worst_removal(self, solution, ratio):
        ...
        【必须自己实现以下算法】：
        1. 收集所有非0节点（跳过长度<3的路径）
        2. 计算每个节点的边际贡献：contrib = dist_contrib + demand * 0.01
           - dist_contrib = dist[prev][node] + dist[node][next] - dist[prev][next]
           - demand从 self.customer_lookup[node]['demand'] 获取
        3. 按contrib降序排序，选择前n个节点
        4. 按(route_idx, pos_idx)降序移除，删除空路径
    ...
    def related_removal(self, solution, ratio):
        ...
        1. 收集所有非0节点
        2. 随机选择一个种子节点
        3. 计算其他节点与种子节点的距离（使用self.dist_matrix）
        4. 按距离升序排序，选择最近的n个节点
        5. 按(route_idx, pos_idx)降序移除，删除空路径
    ...
    def shaw_removal(self, solution, ratio):
        ...
        【必须自己实现以下算法】：
        1. 收集所有非0节点，随机选择种子节点
        2. 计算归一化因子（**关键**：安全归一化 if max_value < 0.001: max_value = 1.0）
        3. 对每个节点计算Shaw相似度：
           shaw_score = 9*normalized_dist + 3*time_diff + 2*demand_diff
        4. 按shaw_score升序排序，选择前n个最相似的节点
        5. 按(route_idx, pos_idx)降序移除，删除空路径
        6. 返回 new_solution, removed_nodes
        '''
        # TODO: 在此处实现算法
    ...
    def history_removal(self, solution, ratio):
        ...
        【必须自己实现以下算法】：
        1. 收集所有非0节点
        2. 检查 self.node_history 是否存在，如果为空则回退到 random_removal
        3. 为每个节点获取 self.node_history.get(node, 0) 值
        4. 按history值升序排序，选择前n个（最久未改动）
        5. 按(route_idx, pos_idx)降序移除，删除空路径
        6. 返回 new_solution, removed_nodes
        '''
        # TODO: 在此处实现算法
    ...
    def cluster_removal(self, solution, ratio):
        ...
        【必须自己实现以下算法】：
        1. 收集所有节点及其坐标 (x, y)
        2. 创建 5x5 网格，根据 (x, y) 将节点分配到网格
        3. 随机选择一个非空网格
        4. 移除该网格中的节点（最多 n 个）
        5. 如果节点不足，扩展到相邻 8 个网格
        6. 按(route_idx, pos_idx)降序移除，删除空路径
        7. 返回 new_solution, removed_nodes
        '''
        # TODO: 在此处实现算法
        pass
    ...
    def greedy_insert(self, solution, removed_nodes):
        ...
        贪心插入节点到成本增量最小的位置。
        '''
        # 【关键：必须检查容量：if current_load+demand>capacity: continue】
    ...
    def regret_insert(self, solution, removed_nodes):
        ...
        后悔修复：优先插入后悔值大的节点。
        '''
        # 【关键：regret=costs[1][0]-costs[0][0]，必须检查容量】
        if not removed_nodes:
            return solution
    ...
    def random_insert(self, solution, removed_nodes):
        ...
        随机修复：随机选择可行位置插入节点。
        '''
        # 【关键：收集满足容量的positions，random.choice(positions)】
        if not removed_nodes:
            return solution
