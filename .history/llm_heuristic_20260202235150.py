...
def extract_solution_from_output(output):
    """从求解输出中提取路线信息"""
    solution = []
    route_pattern = r'Route \d+: \[(.*?)\]'
    matches = re.findall(route_pattern, output)
    
    for match in matches:
        # 提取节点ID
        nodes_str = match.strip()
        if nodes_str:
            nodes = [int(x.strip()) for x in nodes_str.split(',') if x.strip().isdigit()]
            if nodes:
...
def extract_iteration_history(output):
    ...
    iter_pattern = r'Iter\s+(\d+):\s+Current=(\d+\.?\d*),\s+Best=(\d+\.?\d*)'
...
def visualize_results(dataset, solution, best_cost, output):
    ...
    if cost_history and best_history:
        ...
        ax1.plot(iterations, cost_history, 'b-', alpha=0.3, linewidth=1, label='Current Cost')
        ax1.plot(iterations, best_history, 'r-', linewidth=2, label='Best Cost')
        ax1.set_xlabel('Iteration', fontsize=12)
        ax1.set_ylabel('Cost', fontsize=12)
        ax1.set_title('ALNS Convergence History', fontsize=14, fontweight='bold')
        ax1.legend(fontsize=10)
        ax1.grid(True, alpha=0.3)
    else:
        ax1.text(0.5, 0.5, 'No iteration data available', 
    ...
    # 绘制仓库
    ax2.plot(depot_x, depot_y, 'rs', markersize=15, label='Depot', zorder=5)
    
    # 绘制客户点
    customer_coords = [(c['x'], c['y']) for c in customers[1:]]
    if customer_coords:
        cx, cy = zip(*customer_coords)
        ax2.plot(cx, cy, 'b.', markersize=8, label='Customers', zorder=3)
    
    # 绘制路线
    colors = plt.cm.tab20(np.linspace(0, 1, len(solution)))
    for idx, route in enumerate(solution):
        route_coords = [(customers[node]['x'], customers[node]['y']) for node in route]
        if route_coords:
            rx, ry = zip(*route_coords)
            ax2.plot(rx, ry, '-', color=colors[idx], linewidth=2, alpha=0.7, zorder=2)
            ax2.plot(rx, ry, 'o', color=colors[idx], markersize=6, alpha=0.7, zorder=4)
    
    ax2.set_xlabel('X Coordinate', fontsize=12)
    ax2.set_ylabel('Y Coordinate', fontsize=12)
    ax2.set_title(f'Vehicle Routes (n={len(solution)})', fontsize=14, fontweight='bold')
    ...
    if op_stats:
        ...
        success_rates = [op_stats[name]['rate'] for name in op_names]
        
        bars = ax3.barh(op_names, success_rates, color='steelblue', alpha=0.7)
        ax3.set_xlabel('Success Rate (%)', fontsize=12)
        ax3.set_title('Operator Performance', fontsize=14, fontweight='bold')
        ...
        ax3.text(0.5, 0.5, 'No operator statistics available', 
                ha='center', va='center', transform=ax3.transAxes)
        ax3.set_title('Operator Performance', fontsize=14, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig('vrp_result.png', dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    if is_solve_success:
        ...
        # Visualize results
        if solution and best_cost:
            print("\n[Visualization] Generating plots...")
            visualize_results(dataset, solution, best_cost, output)
            print("[Visualization] Plots saved to 'vrp_result.png'")
