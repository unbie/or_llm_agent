...
class HeuristicPlugin:
    ...
    def worst_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 【必须实现】worst_removal算子 - 不能简单调用random_removal！
        # 【关键改进】必须加入需求因子，提升算子成功率：
        # 1. 计算边际贡献时，公式为：contrib = dist_contrib + demand * 0.01
        # 2. dist_contrib = dist[prev][node] + dist[node][next] - dist[prev][next]
        # 3. demand从customer_lookup[node]['demand']获取
        # 4. 按contrib降序排序，选择贡献最大的n个节点移除
        # 实现以下完整逻辑（不要删除）：
    ...
    def related_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 【必须实现】related_removal算子 - 禁止调用random_removal！
        # 【完整步骤】必须按以下逻辑实现：
        # 1. 随机选择种子节点：seed_node = random.choice(all_nodes)[2]
        # 2. 计算所有节点到种子的距离：self.dist_matrix[seed_node][node]
        # 3. 按距离升序排序，选择距离最近的n个节点
        # 4. 移除这些地理位置相近的节点，便于后续重新优化
        # 这是ALNS的核心算子，不能简化！必须完整实现以下逻辑：
    ...
    def shaw_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 【必须实现】shaw_removal算子 - 这是最重要的算子之一！
        # 【关键】必须实现安全归一化，避免除零错误：
        # 1. 计算max_distance, max_time_diff, max_demand
        # 2. 每个最大值检查：if max_value < 0.001: max_value = 1.0
        # 3. Shaw分数 = 9*normalized_dist + 3*time_diff + 2*demand_diff
        # 4. 按Shaw分数升序排序，选择相似度最高的n个节点
        # 【禁止】不能简化为random_removal！必须实现完整的三维相似度计算：
    ...
    def history_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 【必须实现】history_removal算子 - 探索搜索盲区的关键！
        # 【实现步骤】：
        # 1. 检查self.node_history是否为空，如果为空则回退到random_removal
        # 2. 为每个节点获取历史值：history = self.node_history.get(node, 0)
        # 3. 按历史值升序排序（值越小表示越久未修改）
        # 4. 选择历史值最小的n个节点移除
        # 【注意】history为空时可以回退，但有历史记录时必须完整实现！
    ...
    def cluster_removal(self, solution, ratio):
        ...
        Returns:
            (移除后的解, 被移除的节点ID列表)
        '''
        # TODO: 【必须实现】cluster_removal算子 - 空间聚类的重要方法！
        # 【完整实现步骤】：
        # 1. 创建5x5网格：grid_size = 5
        # 2. 计算每个节点的网格坐标：gx = int((x-min_x)/(max_x-min_x)*5)
        # 3. 随机选择一个非空网格单元
        # 4. 如果该网格节点数不足n，扩展到相邻8个网格
        # 5. 移除该聚类区域的n个节点
        # 【禁止】不能简化！必须实现完整的网格聚类逻辑：
    ...
    def greedy_insert(self, solution, removed_nodes):
        ...
        Returns:
            插入所有节点后的完整解
        '''
        # TODO: 【必须实现】greedy_insert算子 - 带容量约束检查！
        # 【关键】必须检查容量约束，否则会生成不可行解：
        # 在for route_idx, route in enumerate(new_solution)循环中：
        #   current_load = sum(customer_lookup[n]['demand'] for n in route if n!=0)
        #   if current_load + node_demand > capacity: continue
        # 【步骤】：
        # 1. 对每个待插入节点，遍历所有现有路径
        # 2. 检查容量约束（跳过容量不足的路径）
        # 3. 计算每个位置的成本增量，选择最小的
    ...
    def regret_insert(self, solution, removed_nodes):
        ...
        Returns:
            插入所有节点后的完整解
        '''
        # TODO: 【必须实现】regret_insert算子 - 提升插入质量的关键！
        # 【关键】必须检查容量约束：
        # 在收集插入位置时：
        #   current_load = sum(customer_lookup[n]['demand'] for n in route if n!=0)
        #   if current_load + node_demand > capacity: continue
        # 【后悔值计算】：
        # 1. 对每个节点，找到所有满足容量的插入位置及成本
        # 2. 排序后取最佳和次佳：regret = costs[1][0] - costs[0][0]
        # 3. 优先插入后悔值最大的节点（不插入代价最高）
        # 4. 重复直到所有节点插入完成
        # 实现以下完整逻辑：
    ...
    def random_insert(self, solution, removed_nodes):
        ...
        Returns:
            插入所有节点后的完整解
        '''
        # TODO: 【必须实现】random_insert算子 - 增加解多样性！
        # 【关键】必须检查容量约束：
        # 在收集可行位置时：
        #   current_load = sum(customer_lookup[n]['demand'] for n in route if n!=0)
        #   if current_load + node_demand > capacity: continue
        # 【步骤】：
        # 1. 收集所有满足容量约束的可行插入位置
        # 2. 将新建路径作为一个选项加入positions列表
        # 3. 从positions中随机选择一个位置插入
        # 4. 这样既保证可行性，又增加多样性
        # 实现以下完整逻辑：
