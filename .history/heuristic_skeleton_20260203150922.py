...
class HeuristicSolver:
    ...
    def solve(self, max_iters=300, seed=None):
        ...
        # ALNS参数 - 根据迭代次数自适应调整
        T = max(current_cost * 0.05, 50) if current_cost < float('inf') else 100  # 初始温度（能接受差5%的解）
        
        # 自适应冷却系数：确保温度在80%迭代后仍有探索能力
        # alpha = (T_final / T_initial) ^ (1 / max_iters)
        # 目标：在max_iters的80%处，温度降到初始的10%
        target_ratio = 0.10  # 最终温度为初始温度的10%
        alpha = target_ratio ** (1.0 / (max_iters * 0.8))
        ...
        for iteration in range(max_iters):
            ...
            # ===== LLM智能权重调整 =====
            # 每50次迭代，让LLM分析性能并调整权重
            if (iteration + 1) % 50 == 0 and hasattr(self, 'llm_weight_adjuster'):
                try:
                    print(f"\n[LLM分析] 第{iteration+1}次迭代，请求LLM分析算子性能...")
                    ...
                    solver_state = {
                        ...
                        'destroy_stats': [],
                        'repair_stats': [],
                        'cost_history': self.best_cost_history[-50:] if len(self.best_cost_history) >= 50 else self.best_cost_history
                    }
                    
                    # 收集破坏算子统计
                    for i, name in enumerate(['random', 'worst', 'related', 'shaw', 'history', 'cluster']):
                        solver_state['destroy_stats'].append({
                            'name': name,
                            'uses': self.op_stats['destroy'][i]['uses'],
                            'success': self.op_stats['destroy'][i]['success'],
                            'weight': self.d_weights[i]
                        })
                    
                    # 收集修复算子统计
                    for i, name in enumerate(['greedy', 'regret', 'random']):
                        solver_state['repair_stats'].append({
                            'name': name,
                            'uses': self.op_stats['insert'][i]['uses'],
                            'success': self.op_stats['insert'][i]['success'],
                            'weight': self.i_weights[i]
                        })
                    
                    # 调用LLM
                    suggestion = self.llm_weight_adjuster(solver_state)
                    
                    if suggestion:
                        print(f"[LLM建议] {suggestion.get('reasoning', '无')}")
                        print(f"[LLM策略] {suggestion.get('strategy', '无')}")
                        
                        # 应用权重调整
                        d_adjustments = suggestion.get('destroy_weight_adjustments', [1.0] * 6)
                        r_adjustments = suggestion.get('repair_weight_adjustments', [1.0] * 3)
                        
                        for i in range(len(self.d_weights)):
                            self.d_weights[i] *= d_adjustments[i]
                        
                        for i in range(len(self.i_weights)):
                            self.i_weights[i] *= r_adjustments[i]
                        ...
                        print(f"[权重更新] Destroy: {[f'{w:.2f}' for w in self.d_weights]}")
                        print(f"[权重更新] Repair: {[f'{w:.2f}' for w in self.i_weights]}\n")
                        
                except Exception as e:
                    print(f"[LLM异常] 权重调整失败: {e}")
