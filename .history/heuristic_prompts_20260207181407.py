...
【成本计算（极其重要，必须遵守）】：
⚠️ 禁止在 greedy_insert / regret_insert 中使用距离增量 dist(a,c)+dist(c,b)-dist(a,b) 作为插入成本！
⚠️ 必须使用 self.solver.calculator.calculate_route_cost() 来计算插入增量！

正确用法（注意需要传两个参数：route 和 self.dist_matrix）：
...
self.solver 由框架在初始化后自动注入到 plugin，在算子被调用时已经可用。
calculate_route_cost(route, dist_matrix) 返回字典，包含 'variable_cost'（综合成本＝运输+冷藏+新鲜度损失+时间窗惩罚）。
...
**【必须使用 self.solver.calculator 计算成本，禁止用距离增量】**
