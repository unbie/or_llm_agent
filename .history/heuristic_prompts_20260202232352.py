...
【worst_removal】- 移除成本贡献最大的节点：
   - **简化策略：以距离为主，需求为辅**
       ```python
       contrib = dist_contrib  # 先纯用距离
       ```
       或者如果要考虑需求：
       ```python
       contrib = dist_contrib + demand * 0.2  # 0.2让距离主导
       ```
   
   - **实现逻辑**：
       1. 计算每个节点的边际贡献：dist_contrib = dist[prev][node] + dist[node][next] - dist[prev][next]
       2. 可选加入轻微需求因子：contrib = dist_contrib (或 + demand * 0.2)
       3. 按contrib降序排序，移除最大的ratio比例
...
【related_removal】（关联破坏）- 简化版：
   - **策略**：随机选一个seed节点，移除离它最近的ratio个节点
   - 实现：
     1. 随机选seed_node
     2. 计算所有节点到seed的距离
     3. 按距离升序排序，选前 ratio 个
     4. 降序移除

【shaw_removal】（Shaw破坏）- 简化版：
   - **策略**：只考虑距离相似性，忽略时间窗和需求差异（减少复杂度）
   - 实现：
     1. 随机选seed_node
     2. 计算距离：dist[seed][node]
     3. 按距离升序排序，选前 ratio 个
     4. 降序移除
   - 注意：这和related_removal几乎一样，但保留是为了算子多样性
...
【cluster_removal】（聚类破坏）- 简化版：
   - **策略**：随机选一个节点，移除附近区域的节点（与related类似）
   - 实现：
     1. 随机选seed_node
     2. 计算所有节点到seed的距离
     3. 按距离升序排序，选前 ratio 个
     4. 降序移除
   - 注意：为保持算子多样性，可以用不同seed选择策略
    ...
    def worst_removal(self, solution, ratio):
        ...
        for route_idx, route in enumerate(solution):
            ...
            for pos_idx in range(1, len(route)-1):
                ...
                contrib = dist_contrib  # 或 dist_contrib + demand * 0.2
