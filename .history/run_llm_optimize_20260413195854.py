...
import io
import sys
python run_llm_optimize.py --type c1 --dataset c101
...
# 主优化循环
# ============================================================
def run_optimization(dataset_path, dataset_type="c1", max_rounds=3,
    ...
    best_solution, best_cost, metrics = run_solver(
        data, current_code, max_iter=iters_per_round, seed=seed
    )
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        new_solution, new_cost, new_metrics = run_solver(
            data, new_code, max_iter=iters_per_round, seed=seed
        )
        ...
        history.append({
            ...
            'prev_cost': prev_cost,
            'change': cost_change,
            'summary': analysis[:200] if analysis else '(无分析)',
        })
    ...
    final_solution, final_cost, final_metrics = run_solver(
        data, best_ever_code, max_iter=final_iters, seed=seed
    )

    if final_metrics:
        print(f"\n[最终结果]")
        print(f"  基线成本:   {baseline_cost:.2f}")
