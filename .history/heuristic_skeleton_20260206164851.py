...
class HeuristicSolver:
    ...
    def cost(self, solution):
        ...
        for route_ids in solution:
            ...
            # 分项累加（用于调试）
            dist = route_cost_info['dist']
            total_c12 += dist * self.calculator.c
            total_c13 += (route_cost_info['variable_cost'] - route_cost_info['c2'] - route_cost_info['c3'] - dist * self.calculator.c)
            total_c2 += route_cost_info['c2']
            total_c3 += route_cost_info['c3']
        
        # 添加调试信息（每100次迭代打印一次）
        if hasattr(self, 'current_iteration') and self.current_iteration % 100 == 0:
            print(f"  [成本调试] C11={fixed_cost:.2f}, C12={total_c12:.2f}, C13={total_c13:.2f}, C2={total_c2:.2f}, C3={total_c3:.2f}, Total={fixed_cost + total_variable_cost:.2f}")
