...
【生成代码后必须自检】：
✅ worst_removal: 是否包含 `contrib = dist_contrib + demand * 0.5` （必须是0.5不是0.01）
✅ greedy_insert: 是否使用 `<=` 比较（不是 `<`）
✅ greedy_insert: best_cost 是否初始化为 `float('inf')`
✅ 所有removal: 是否按 (route_idx, pos_idx) 降序删除
✅ 所有算子: 是否处理空列表边界情况
✅ 所有算子: 是否使用 `n = min(n, len(nodes))` 防止越界
...
【worst_removal】- 成功率提升的关键算子：
   - **核心公式（必须严格遵守）**：
       ...
       contrib = dist_contrib + demand * 0.5  # ← 必须加需求因子！权重0.5很关键
   - **为什么用0.5作为需求权重**：
     * 0.01太小，需求因素几乎不起作用（经验证效果差）
     * 0.5让需求和距离平衡：既考虑地理位置，也优先移除高需求节点
     * 移除高需求节点后，修复时有机会重新分配到更优路径，减少车辆数
     * 这个权重在Solomon基准测试中效果最佳
    ...
    def worst_removal(self, solution, ratio):
        ...
        for route_idx, route in enumerate(solution):
            ...
            for pos_idx in range(1, len(route)-1):
                ...
                # 【必须实现】添加需求因素，权重0.5不可修改！
                demand = self.customer_lookup.get(node, {}).get('demand', 0)
                contrib = dist_contrib + demand * 0.5  # ← 权重0.5是关键，不要改成0.01！
...
【代码完成后务必检查以下关键点】：
1. worst_removal 的 contrib 公式是否包含 `+ demand * 0.5` （注意：必须是0.5，不是0.01！）
