    ...
    if solution and len(solution) > 0:
        ...
        for idx, route in enumerate(solution):
            try:
                ...
                # 如果路线没有以0开始或结束，添加仓库节点
                complete_route = list(route)
                if complete_route[0] != 0:
                    complete_route.insert(0, 0)
                ...
                for node in complete_route:
                    if node < len(customers):
                        route_coords.append((customers[node]['x'], customers[node]['y']))
                ...
                if route_coords and len(route_coords) >= 2:
                    ...
                    # 绘制完整路线（闭合回路）
                    ax2.plot(rx, ry, '-', color=color, linewidth=2.5, alpha=0.75, zorder=2, label=f'路线{idx+1}')
                    
                    # 绘制中间的客户点（不包括起始和结束的仓库）
                    if len(rx) > 2:
                        ax2.scatter(rx[1:-1], ry[1:-1], c=[color]*(len(rx)-2), s=80, alpha=0.9, 
                                   zorder=4, edgecolors='black', linewidths=1.5)
                    
                    # 标注路线编号
                    mid_idx = len(rx) // 2
                    ax2.text(rx[mid_idx], ry[mid_idx], f'{idx+1}', 
