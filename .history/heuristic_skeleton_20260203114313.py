...
class HeuristicSolver:
    ...
    def solve(self, max_iters=300, seed=None):
        ...
        print(f"\n{'='*60}")
        print("[Result] Best Cost: " + (f"{best_cost:.2f}" if best_cost < float('inf') else "INF"))
        print(f"[Result] Number of Routes: {len(best_solution)}")
        print(f"[Result] Improvements: {improvement_count}")
        print(f"[Result] Last Improvement at Iteration: {last_improve_iter}")
        print(f"{'='*60}\n")
    ...
    def solve_multi_run(self, max_iters=300, num_runs=3, base_seed=42):
        ...
        print(f"  Best Cost: {best_overall:.2f}")
        print(f"  Average Cost: {avg_cost:.2f}")
        print(f"  Number of Routes: {len(best_sol_overall)}")
        print(f"  Individual Costs: {[f'{c:.2f}' for c in all_costs]}")
        print(f"{'='*60}\n")
