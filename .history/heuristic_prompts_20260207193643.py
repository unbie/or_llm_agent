...
【成本计算方式】：
   ...
   1. **简化距离成本**（推荐在算子中使用）：
      cost_inc = dist(prev, node) + dist(node, next) - dist(prev, next)
      - 优点：计算快速，适合ALNS大量迭代
      - 缺点：不考虑时间窗、货损等复杂因素
   
   2. **完整成本计算**（通过 self.solver.calculator 调用）：
      # 计算路径完整成本（C12距离+C13制冷+C2货损+C3时间惩罚）
      route_nodes = [self.solver.id_to_customer[nid] for nid in route]
      cost_info = self.solver.calculator.calculate_route_cost(route_nodes, self.dist_matrix)
      total_cost = cost_info['variable_cost']  # 返回C12+C13+C2+C3总成本
      - 优点：精确考虑所有成本因素
      - 缺点：计算较慢
   
   **建议**：算子中使用方法1（距离增量），最终解评估通过框架的cost()函数自动使用方法2
...
**成本计算**：同greedy_insert，推荐使用距离增量（方法1）
...
【关于成本计算函数的说明】：
utils.py 中的 calculate_route_cost() 函数会在框架的 cost() 函数中自动调用，
用于计算最终解的真实成本（包括C12距离+C13制冷+C2货损+C3时间惩罚）。
在算子实现中，你可以选择：
- 使用简化的距离增量计算（推荐，速度快）
- 或通过 self.solver.calculator.calculate_route_cost() 使用完整成本计算（更精确但更慢）
"""
