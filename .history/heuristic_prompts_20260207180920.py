...
【成本计算（重要）】：
默认应使用 utils.py 中的成本计算（FreshnessAndPenaltyCalculator）。
在算子内如需计算插入增量，请优先使用：
    old_cost = self.solver.calculator.calculate_route_cost(route)['variable_cost']
    new_cost = self.solver.calculator.calculate_route_cost(new_route)['variable_cost']
    delta = new_cost - old_cost
若 self.solver 或 calculator 不可用，再退化为距离增量：
    dist(a,c) + dist(c,b) - dist(a,b)
注意：HeuristicSolver 会把自身注入到 plugin：self.solver 可用。
...
**【必须使用真实成本函数】**：
通过 `self.solver.calculator.calculate_route_cost(route)` 计算成本，
**不要使用** `self.dist_matrix[a][b] + self.dist_matrix[b][c] - self.dist_matrix[a][c]` 作为增量！
    ...
    for node in list(removed_nodes):
        ...
        for route_idx, route in enumerate(solution):
            ...
            if current_load + node_demand > capacity:
                continue
            # 当前路径真实成本
            old_cost = self.solver.calculator.calculate_route_cost(route, self.dist_matrix)['variable_cost']
            for pos in range(1, len(route)):
                new_route = route[:pos] + [node] + route[pos:]
                new_cost = self.solver.calculator.calculate_route_cost(new_route, self.dist_matrix)['variable_cost']
        ...
        # 新建路径的成本
        new_route_cost = self.solver.calculator.calculate_route_cost([0, node, 0], self.dist_matrix)['variable_cost']
...
class HeuristicPlugin:
    def __init__(self, **kwargs):  
        ...
        self.dist_matrix = None  # 由框架注入
        self.solver = None       # 由框架注入，可通过 self.solver.calculator 访问成本计算器
