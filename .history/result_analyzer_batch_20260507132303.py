...
def load_results(results_dir="experiments_batch/results", only_core30=True):
    """加载所有实验结果"""
    results = []
    ...
    for result_file in results_path.glob("*.json"):
        try:
            with open(result_file, 'r', encoding='utf-8') as f:
                ...
                if only_core30 and instance not in CORE30_INSTANCES:
                    continue
...
def generate_figures(df, output_dir="experiments_batch"):
    ...
    # 设置中文字体和学术风格
    plt.rcParams.update({
        'font.sans-serif': ['Microsoft YaHei', 'SimHei', 'Arial Unicode MS'],  # 中文字体
...
def generate_sensitivity_figures(df, output_dir="experiments_batch/figures"):
    ...
    ax.set_xlabel('theta1 (Transport Deterioration Rate)')
    ax.set_ylabel('Total Cost')
    ax.set_title('Cost Sensitivity to Deterioration Parameter theta1', fontweight='bold')
    ax.legend()
    ax.grid(alpha=0.3)
