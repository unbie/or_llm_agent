...
1. **智能破坏**：
   - `_random_removal`: 基于时间窗紧张度加权采样，优先移除时间窗紧张的节点
   - `_route_removal`: 按成本效益（单位客户成本）排序，优先移除低效路径
   - `_string_removal`: 优先移除地理位置连续的节点（距离最近的连续节点）

2. **计算优化**：
   - 修复算子使用 `_candidate_positions` 大幅减少位置评估数量
   - 引入路径成本缓存，避免重复计算 `calculate_route_cost`
   - `_regret_insert` 扩展为 Regret-3，更好评估插入机会成本

3. **针对性策略**：
   - 破坏算子考虑时间窗和成本结构
   - 修复算子优先考虑时间窗友好的插入位置

这些改进能显著提高算子成功率，减少计算开销，并针对生鲜物流的时间敏感特性进行优化。