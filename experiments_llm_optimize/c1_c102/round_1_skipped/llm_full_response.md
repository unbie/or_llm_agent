...
1. **破坏算子**：
   - `_random_removal`: 引入加权随机选择，优先移除时间窗紧张或货损成本高的节点
   - `_route_removal`: 优先移除时间窗违规严重或货损成本高的不良路径
   - `_string_removal`: 优先移除时间窗连续紧张的节点段

2. **修复算子**：
   - `_greedy_insert`: 使用`_candidate_positions`减少位置评估，按时间窗紧迫度排序插入顺序，尝试合并单客户路径
   - `_regret_insert`: 使用Regret-3计算，考虑时间窗紧迫度权重，按紧迫度预排序节点
...
这些改进针对性地解决了生鲜VRP的时间敏感特性，预计能降低C2和C3成本，提高算子成功率。