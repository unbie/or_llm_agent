...
1. **破坏算子**：
   - `_random_removal`：改为成本敏感，优先移除边际成本高的节点
   - `_route_removal`：改为货损敏感，优先移除货损成本高的路径
   - `_string_removal`：改为时间窗连续移除，移除时间窗相近的节点

2. **修复算子**：
   - `_greedy_insert`：使用增量成本估算和候选位置，大幅减少计算量
   - `_regret_insert`：扩展为Regret-3，使用加权后悔值和增量估算

3. **关键创新**：
   - 破坏时考虑修复难度（边际成本）
   - 修复时使用增量估算而非完整重算
   - 充分利用问题特性（时间窗、货损成本）
   - 避免历史记录中的无效策略，采用全新的增量计算思路