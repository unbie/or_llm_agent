...
def run_batch_experiments():
    ...
    # Solomon数据集类型和实例 - 快速测试版（只测试2个数据集）
    experiments = [
        # (数据集类型, 实例名, 运行次数)
        ('c1', 'c101.txt', 2),  # 快速测试: 只测2次
        ('r1', 'r101.txt', 2),  # 快速测试: 只测2次
    ]
    ...
    for dataset_type, instance, num_runs in experiments:
        ...
        for run_id in range(1, num_runs + 1):
            ...
            result = run_single_experiment(str(dataset_path), max_iters=100, seed=seed)  # 快速测试: 100次迭代
