...
用法:
    ...
    solver = ALNSVRPSolver(data, max_iter=3000, seed=42)
...
class ALNSVRPSolver:
    ...
    def _is_route_feasible(self, route_ids):
        load = 0
        ...
        for node in removed_nodes:
            ...
            for ri, route in enumerate(new_sol):
                ...
                for pos in self._candidate_positions(route, node):
                    ...
                    if not self._is_route_feasible(route_after):
                        continue
            ...
            # 新路径成本
            nr = [0, node, 0]
            if not self._is_route_feasible(nr):
                continue
            ...
            for node in remaining:
                ...
                for ri, route in enumerate(new_sol):
                    ...
                    for pos in self._candidate_positions(route, node):
                        ...
                        if not self._is_route_feasible(ra):
                            continue
    ...
    def _build_initial_solution(self):
        ...
        # 初始解构建时间上限（秒），超时则回退到“每客户一车”
        build_deadline = time.time() + 60
        ...
        for strat in strategies:
            ...
            while remaining:
                if time.time() > build_deadline:
                    if self.verbose:
                        print("[Init] Timeout building initial solution, fallback to one-customer routes")
                    fallback = [[0, c['id'], 0] for c in non_depot]
                    return fallback, self.cost(fallback)
                route = [0]
                load = 0
                pos_id = 0
                curr_time = 0.0
                while remaining:
                    ...
                    if best_cust is None:
                        if len(route) == 1:
                            # 无法在当前条件下找到可插入客户，强制单客户成车避免死循环
                            fallback_cust = min(remaining, key=lambda x: x.get('due_date', float('inf')))
                            route.extend([fallback_cust['id'], 0])
                            solution.append(route)
                            remaining.remove(fallback_cust)
                            route = [0]
                            load = 0
                            pos_id = 0
                            curr_time = 0.0
    ...
    def solve(self):
        ...
        if current_cost == float('inf'):
            ...
            current_cost += infeasible_penalty

        best_solution = copy.deepcopy(current_solution) if is_feasible else None
        best_cost = current_cost if is_feasible else float('inf')
        best_feasible_solution = copy.deepcopy(best_solution) if best_solution is not None else None
        best_feasible_cost = best_cost

        # 温度参数 (与 heuristic_skeleton 一致)
        T = max(current_cost * 0.04, 100) if current_cost < float('inf') else 100
        target_ratio = 0.005
        alpha = target_ratio ** (1.0 / self.max_iter)

        segment_size = max(50, int(self.max_iter * 0.10))
        restart_threshold = max(60, int(self.max_iter * 0.35))
        ...
        for it in range(self.max_iter):
            if time.time() - start_time >= self.time_budget_sec:
                if self.verbose:
                    print(f"[Stop] Time budget reached ({self.time_budget_sec}s)")
                break
            # 自适应破坏程度
            thr_med = self.max_iter * 0.27
            thr_high = self.max_iter * 0.53
            if no_improve > thr_high:
                base_rm = max(3, int(len(non_depot) * 0.18))
                max_rm = max(5, int(len(non_depot) * 0.28))
            elif no_improve > thr_med:
                base_rm = max(2, int(len(non_depot) * 0.10))
                max_rm = max(4, int(len(non_depot) * 0.16))
            else:
                base_rm = max(2, int(len(non_depot) * 0.06))
                max_rm = max(3, int(len(non_depot) * 0.10))
            ...
            try:
                ...
                if removed:
                    ...
                    new_sol = partial
                feasible = self.validate(new_sol)
                new_cost = self.cost(new_sol)
                if not feasible:
                    new_cost += infeasible_penalty
            except Exception:
                new_cost = float('inf')
            ...
            if new_cost < current_cost:
                ...
                if feasible and new_cost < best_cost:
                    ...
                    best_cost = new_cost
                    best_feasible_solution = copy.deepcopy(new_sol)
                    ...
                    if feasible and new_cost < best_feasible_cost:
                        best_feasible_solution = copy.deepcopy(new_sol)
                        best_feasible_cost = new_cost
        ...
        if best_feasible_solution is None:
            best_feasible_solution = [[0, c['id'], 0] for c in non_depot]
            best_feasible_cost = self.cost(best_feasible_solution)

        best_solution = copy.deepcopy(best_feasible_solution)
        best_cost = best_feasible_cost

        # 后处理 2-opt（仅接受可行改进）
        for idx in range(len(best_solution)):
            best_solution[idx] = self._two_opt(best_solution[idx])
        if self.validate(best_solution):
