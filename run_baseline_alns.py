...
用法:
  ...
  # 调整参数:
  python run_baseline_alns.py --all --maxiter 3000
"""
...
def quick_test(
    ...
    output_dir: str = 'experiments_alns_baseline',
    runs: int = 3,
    overwrite_if_better: bool = True,
):
    """快速测试单个算例，多次运行取最低；仅在更优时覆盖 run1 结果。"""
    ...
    existing_files = sorted(results_dir.glob(f"{dataset_type}_{instance}_run*.json"))
    existing_best = None
    ...
    for existing_file in existing_files:
        try:
            ...
            existing_cost = existing_data.get('best_cost')
            if existing_cost is not None and (existing_best is None or existing_cost < existing_best):
                existing_best = existing_cost
                existing_best_file = existing_file
        except Exception:
            continue

    if existing_best is not None:
        print(f"[Existing] best_cost={existing_best:.2f} from {existing_best_file.name}")
    else:
        print("[Existing] no prior valid result found")

    run_results = []
    for run_id in range(1, runs + 1):
        seed = run_id * 42
        print(f"\n[Run {run_id}/{runs}] seed={seed}")
        result = run_single_alns_experiment(
            dataset_path=str(dataset_path),
            max_iter=max_iter,
            seed=seed,
        ...
        run_results.append((run_id, seed, result))

        if result['success']:
            print(f"[Run {run_id}] Cost={result['best_cost']:.2f} | Routes={result['num_routes']} | Time={result['elapsed_time']:.1f}s")
        else:
            print(f"[Run {run_id}] FAILED: {result['output'][:200]}")

    successful_runs = [(run_id, seed, result) for run_id, seed, result in run_results if result['success'] and result['best_cost'] is not None]
    if not successful_runs:
        print("[Result] no successful run; existing result not changed")
        return

    best_run_id, best_seed, best_result = min(successful_runs, key=lambda item: item[2]['best_cost'])
    best_cost = best_result['best_cost']

    print(f"\n[Best of {runs}] run={best_run_id}, seed={best_seed}, cost={best_cost:.2f}, routes={best_result['num_routes']}")

    should_write = existing_best is None or best_cost < existing_best
    if overwrite_if_better and not should_write:
        print(f"[Keep] new best {best_cost:.2f} is not lower than existing {existing_best:.2f}; no file overwritten")
        return
    ...
    result_data = {
        ...
        'n_customers':      best_result['n_customers'],
        'total_iterations': best_result['total_iterations'],
        'selected_from_runs': runs,
        'selection_policy':  'best_of_runs_overwrite_if_better',
    }
    with open(result_file, 'w', encoding='utf-8') as f:
        json.dump(result_data, f, indent=2, ensure_ascii=False)

    if existing_best is None:
        print(f"[Write] no existing result; wrote {result_file}")
    else:
        print(f"[Overwrite] improved {existing_best:.2f} -> {best_cost:.2f}; wrote {result_file}")

    print(f"\n{'-'*60}")
    print(f"BEST_COST:  {best_result['best_cost']}")
    print(f"NUM_ROUTES: {best_result['num_routes']}")
    print(f"ELAPSED:    {best_result['elapsed_time']:.2f}s")
    print(f"{'-'*60}")
...
Examples:
  ...
  # Harder instance
  python run_baseline_alns.py --instance r201 --type r2 --maxiter 3000
  ...
  python run_baseline_alns.py --all --maxiter 3000
    ...
    parser.add_argument('--maxiter',  type=int, default=3000, help='Max ALNS iterations (default 3000)')
    parser.add_argument('--outdir',   default='experiments_alns_baseline', help='Batch output directory')
    parser.add_argument('--runs',     type=int, default=3, help='Single-instance reruns; best result is selected (default 3)')
    parser.add_argument('--force',    action='store_true', help='Overwrite run1 even if the new best is not lower')
    ...
    if args.all:
        ...
        quick_test(
            ...
            output_dir=args.outdir,
            runs=args.runs,
            overwrite_if_better=not args.force,
        )
